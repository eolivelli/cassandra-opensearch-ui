 ...
 */
package com.dbui.store.opensearch;
 ...
 * raw-REST level (rather than pulling in a heavy client library) mirrors the DB UI module and keeps
 * the exact requests visible.
 */
@Component
public class OpenSearchClient {
    ...
    /** Performs a PUT with a JSON body and parses the JSON response. */
    public JsonNode put(String path, String jsonBody) {
        HttpRequest request = baseRequest(path).header("Content-Type", "application/json")
                .PUT(HttpRequest.BodyPublishers.ofString(jsonBody, StandardCharsets.UTF_8)).build();
        return send(request);
    }

    /** Returns the HTTP status code of a HEAD request without throwing on 404. */
    public int head(String path) {
        try {
            HttpResponse<Void> response = httpClient.send(
                    baseRequest(path).method("HEAD", HttpRequest.BodyPublishers.noBody()).build(),
    ...
    /** Convenience accessor for the configured OpenSearch base URL. */
